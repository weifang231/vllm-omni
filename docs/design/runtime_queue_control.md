...
HTTP 429 with error type `AdmissionRejectedError`, while the server continues
serving other requests. Admission requires `policy: "edf"`; operators should
also use mutually consistent class/global limits so runtime ordering and
available capacity match the fitted model.
